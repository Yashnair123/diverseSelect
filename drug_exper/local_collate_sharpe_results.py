...
gammas = ['gamma=0.25', 'gamma=0.5', 'gamma=0.75']
...
for couple in [True]:
    results = \
        pd.read_csv(f'collated_sharpe_results/metrics_c{couple}_q{quantile_indexer}_a{alpha100}.csv', header=None).to_numpy().astype('float')
    vanilla_results = \
        pd.read_csv(f'collated_sharpe_results/vanilla_metrics_c{couple}_q{quantile_indexer}_a{alpha100}.csv', header=None).to_numpy().astype('float')
    
    print("Loading histogram results...")
    histogram_results = []

    with open(f'collated_sharpe_results/histogram_c{couple}_q{quantile_indexer}_a{alpha100}.csv', 'r') as file:
